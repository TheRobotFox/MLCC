...
fn rule_user_0(reductends: Vec<Reductend>, identifier: Rc<str>) -> Rule
{
    ...
    Rule{ identifier, reductends: Reductends{reductends}, export: None}
}

fn rule_user_1(export: Rc<str>, reductends: Vec<Reductend>, identifier: Rc<str>) -> Rule
{

    Rule{ identifier, reductends: Reductends{reductends}, export: Some(export) }
}
...
{
    Reductend{components: Components{components}, code: None}
}
fn reductent_user_1(code: Rc<str>, components: Vec<Component>) -> Reductend
{
    Reductend{components: Components{components}, code: Some(code)}
}
...
pub fn parse(mut lex: logos::Lexer<gTokens>) -> Result<GAst, GError>
{
    ...
    macro_rules! pop_val{
        ($type:tt) => {
            match value_stack.pop() {Some(Types::$type(val)) => val, t@_ => {panic!("Wrong type! {:?}\nstack: {:?}", t, value_stack)}}
        }
    }
    ...
    'a :loop{
        match state {
                ...
                loop{
                    match lex.next() {
                        ...
                        t @ _ => {
                            ...
                            let start = new_lines.last().map_or(0, |s| s.0.clone()+1);
                            let offset = span.start-start;
                            let r = Err(GError{expected: vec![gTokens::Var, gTokens::Identifier], found: t, text: String::from(lex.source().get(start..span.end).unwrap()), line, offset, len: span.end-span.start});
                            panic!("Error: {:?}\nstack: {:?}", r, value_stack);
                            return r;
                        }
                    }
            ...
            States::ReduceRule1 => {
                reduce!(Rule, rule_user_1(pop_val!(Token), pop_val!(VecReductend), pop_val!(Token)));
                state = state_stack.pop().unwrap();
            },
            ...
            States::ReduceC1 => {
                reduce!(VecComponent, components_user_1(pop_val!(Component), pop_val!(VecComponent)));
                state=state_stack.pop().unwrap();
            },
            States::ReduceRN => {
            ...
            States::ReduceRC => {
                reduce!(Reductend, reductent_user_1(pop_val!(Token), pop_val!(VecComponent)));
                state=state_stack.pop().unwrap();
            },
            States::ReduceR1 => {
                c = States::ReduceC0;
                reduce!(VecReductend, reductents_user_1(pop_val!(Reductend), pop_val!(VecReductend)));
                state=state_stack.pop().unwrap();
            },
            States::PushR => {
            ...
            States::Reductends => {
                match_next!(lex, gTokens::Assign, {state=States::Assign},
                                 gTokens::Or,     {
            ...
            States::Generics => {
                match lex.next() {
                    ...
                    None => {
                        let span = lex.span();
                        let source = lex.source().get(..span.end).unwrap_or_else(||panic!("Source is Corrupted! {:?} {:?}", span.end, lex.source().len()));

                        let new_lines: Vec<(usize, &str)> = source.match_indices("\n").collect();
                        let line = new_lines.len()+1;
                        let start = new_lines.last().map_or(0, |s| s.0.clone()+1);
                        let offset = span.start-start;
                        let r = Err(GError{expected: vec![], found: None, text: String::from(source.get(start..).unwrap()), line, offset, len: span.end-span.start});
                        panic!("Error: {:?}\nstack: {:?}", r, value_stack);
                        return r;
                    },
                    _ => {
            ...
            States::RStart => {
                match_next!(lex, gTokens::Identifier, {reduce!(Component0, Component0::Rule(lex.slice().into()))},
                                 gTokens::Terminal,   {reduce!(Component0, Component0::Terminal(lex.slice().into()))},
                                 gTokens::Regex,      {reduce!(Component0, Component0::Regex(lex.slice().into()))},
                                 gTokens::Star,      {reduce!(Component0, Component0::Token)}
                );
            ...
            States::Assign => {
                ...
                reduce!(Component, component_user_1(pop_val!(Token), pop_val!(Component0)));
                state_stack.push(States::RBegin);
                state=c.clone();
            },
            States::CodeE => {
                match_next!(lex, gTokens::Or, {
                     state_stack.push(States::RStart);
                },
                gTokens::Arrow, {
                     state_stack.push(States::ReduceStatement);
